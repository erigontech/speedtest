...
func SelectSegmentFromWebseeds(hosts []string, cloudFareHeader http.Header) (string, error) {
	return defaultClient.SelectSegmentFromWebseeds(hosts, cloudFareHeader)
}

func (s *Speedtest) SelectSegmentFromWebseeds(hosts []string, cloudFareHeaders http.Header) (string, error) {
	for _, host := range hosts {
		trimmedString := strings.TrimPrefix(host, "v1:")
		trimmedString = strings.TrimSuffix(trimmedString, "/") + "/"

		segmentsListArray, err := s.GetSegmentsListFromWebseeds(trimmedString, cloudFareHeaders)
		if err != nil {
			continue
		}

		for _, segment := range segmentsListArray {
			request, err := http.NewRequestWithContext(context.Background(), http.MethodHead, trimmedString+segment, nil)
			if err != nil {
				continue
			}

			insertCloudflareHeaders(request, cloudFareHeaders)
			...
			if num >= 2000000 {
				return trimmedString + segment, nil
			}
		}
	}

	return "", errors.New("no segment found")
}

func (s *Speedtest) GetSegmentsListFromWebseeds(host string, cloudFareHeaders http.Header) ([]string, error) {
	segmentsListArray := make([]string, 0)
	request, err := http.NewRequestWithContext(context.Background(), http.MethodGet, host+"manifest.txt", nil)
	if err != nil {
		return []string{}, err
	}
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return []string{}, fmt.Errorf("http status code: %d", resp.StatusCode)
	}
	...
	for _, f := range fileNames {
		...
		case "manifest.txt":
			continue
		default:
			segmentsListArray = append(segmentsListArray, trimmed)
		}
	}

	return segmentsListArray, nil
}

func insertCloudflareHeaders(req *http.Request, cloudflareHeaders http.Header) {
